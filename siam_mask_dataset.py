...
class SubDataSet(object):
    def __init__(self, cfg):
        ...
        # self.has_mask = self.mark in ['coco', 'ytb_vos', 'my_data']
        self.has_mask = True
...
class DataSets(Dataset):
    ...
    def __getitem__(self, index, debug=False):
        ...
        # print('dataset.has_mask', dataset.has_mask)
        if dataset.has_mask and not neg:
        ...
        # mask = np.expand_dims((mask > 0.5), axis=0)  # 1*H*W
        # mask = np.float64(mask)
        # for i in range(255):
        #     print(mask[0][i])
        # print(delta.shape)
        # print(template.shape, search.shape, cls.shape, delta.shape, delta_weight.shape,)
