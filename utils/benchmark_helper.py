...
def load_dataset(dataset):
    ...
    elif 'segtrack' in dataset and 'TEST' not in dataset:
        print('__________________--------________________')
        base_path = join(realpath(dirname(__file__)), '../data', 'my_data', 'train')
        list_path = join(realpath(dirname(__file__)), '../data', 'my_data', 'train', 'meta.json')
        ...
        for video in videos:
            ...
            if len(info[video]['anno_files']) != len(info[video]['image_files']):
                ...
                print("{}: \nanno_files: {}\nimage_files: {}\n".format(video[1:-1], \
                        len(info[video]['anno_files']), len(info[video]['image_files'])))
        # exit()
    elif 'my_data' in dataset:
        base_path = join(realpath(dirname(__file__)), '../data', 'my_data', 'train')
        json_path = join(realpath(dirname(__file__)), '../data', 'my_data', 'train', 'meta.json')
