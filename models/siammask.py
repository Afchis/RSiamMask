...
class SiamMask(nn.Module):
    ...
    def _add_rpn_loss(self, label_cls, label_loc, lable_loc_weight, label_mask, label_mask_weight,
        ...
        rpn_loss_mask, iou_m, iou_5, iou_7, afchi_mask, afchi_label = select_mask_logistic_loss(rpn_pred_mask, label_mask, label_mask_weight)

        return rpn_loss_cls, rpn_loss_loc, rpn_loss_mask, iou_m, iou_5, iou_7, afchi_mask, afchi_label
    ...
    def forward(self, input):
        ...
        if self.training:
            rpn_loss_cls, rpn_loss_loc, rpn_loss_mask, iou_acc_mean, iou_acc_5, iou_acc_7, afchi_mask, afchi_label = \
            ...
            # Afchi.add:
            outputs['afchi_masks'] = afchi_mask, afchi_label
                # Afchi.add: end
        return outputs
...
def select_mask_logistic_loss(p_m, mask, weight, o_sz=63, g_sz=127):
    ...
    if pos.nelement() == 0: return p_m.sum() * 0, p_m.sum() * 0, p_m.sum() * 0, p_m.sum() * 0, p_m.sum() * 0, p_m.sum() * 0
    ...
    # print(afchi_mask, afchi_label)
    loss = F.soft_margin_loss(p_m, mask_uf)
    iou_m, iou_5, iou_7 = iou_measure(p_m, mask_uf)
    return loss, iou_m, iou_5, iou_7, afchi_mask, afchi_label
